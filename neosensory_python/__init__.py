...
class NeoDevice:
    ...
    async def stop_motors(self):
        """Send a frame that turns off the motors. Note the API CLI command "motors stop" disables the
            ...
            will fail. Assumes this is a 4-motor device (e.g. Neosensory Buzz)
        """
    ...
    async def vibrate_motors(self, motor_list):
        ...
        Args:
            motor_list: a list of motor intensity values on [0,255]. This is API call is designed for
                Neosensory Buzz--and assumes 4 motors. If the list length is > 4, the Buzz will put
                subsequent encodings on the queue to be played out on a 16 ms period.
...
def get_motor_intensity(linear_intensity, min_intensity, max_intensity):
    """Linearly map a value on [0,1] to a motor vibration strenght on [min_intensity, max_intensity]

    Args:
        linear_intensity: an intensity value on [0,1]
        
        min_intensity: a minimum motor intensity value. Typically 0 or minimal perceptible motor activation value.
        
        max_intensity: a maximum motor intensity value. For Neosensory Buzz, motor values can be on [0,255]
...
def get_buzz_illusion_activations(linear_intensity, location,
    ...
    """Apply a haptic illusion across a Neosensory device to create the sensation of a single
        arbitrary point around the wrist vibrating.
